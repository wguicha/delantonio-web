    ...
    <header className="fixed top-0 left-0 right-0 z-50 bg-rock-black/95 backdrop-blur-sm border-b border-rock-border">
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <div className="flex items-center justify-between h-16">
          {/* Logo */}
          <Link to="/" aria-label="Del Antonio - Inicio" className="pl-2">
            <Logo size="sm" />
          </Link>
          ...
          <div className="flex items-center gap-3">
            ...
            <Link
              to="/pedir"
              className="relative flex items-center gap-2 bg-rock-red hover:bg-rock-red-bright text-white px-4 py-2 font-rock tracking-wider text-sm transition-colors"
            ...
            >
              ...
              PEDIR
              {totalItems > 0 && (
                <span className="absolute -top-1.5 -right-1.5 bg-rock-gold text-rock-black text-xs font-bold rounded-full w-5 h-5 flex items-center justify-center" aria-live="polite">
                  {totalItems}
                </span>
              )}
            </Link>
